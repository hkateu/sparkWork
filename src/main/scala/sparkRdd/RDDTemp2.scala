...
case class TempData2(day: Int, doy: Int, month: Int, year: Int,
...
object RDDTempData2 extends App{
  val conf = new SparkConf().setAppName("Temp Data")
    .setMaster("local[*]").set("spark.executor.heartbeatInterval", "200000")
  ...
  val data = lines.flatMap{line =>
    ...
    if(p(7) == "." || p(8) == "." || p(9) == ".") Seq.empty else
           Seq(TempData2(p(0).toInt, p(1).toInt, p(2).toInt, p(4).toInt,
          TempData2.toDoubleOrNeg(p(5)), TempData2.toDoubleOrNeg(p(6)), p(7).toDouble, p(8).toDouble,
          p(9).toDouble))
  }.cache()
  ...
  println(s"There are $rainyCount days. This is ${rainyCount*100.0/data.count()} percent")
  ...
  println(s"The average rainy temperature is ${rainyTemps.sum/rainyTemps.count}")
  ...
  val averageTempsByYear = keyedByYear.aggregateByKey(0.0 -> 0)({case ((sum, cnt), td) => (sum+td.tmax, cnt+1)},
  {case ((s1,c1), (s2,c2)) => (s1+s2, c1+c2)})

  val averageTempsByYearArray = averageTempsByYear.collect()
  val averageTempsByYearData = averageTempsByYearArray.map({case (k,v) => k -> v._1/v._2})
