...
object RDDUnemployment extends App{
  ...
  val areas = sc.textFile("data/laArea.csv").filter(!_.contains("area_type")).map{line =>
  ...
  val series = sc.textFile("data/laSeries.csv").filter(!_.contains("area_code")).map{line =>
    val p = line.split("\t").map(_.trim)
  ...
  val data = sc.textFile("data/minesota.csv").filter(!_.contains("year")).map{line =>
    val p = line.split("\t").map(_.trim)
  ...
  val maxDecade = decadeAverages.map{case((id: String, dec: Int), av: Double) => id -> (dec, av)}.reduceByKey((x,y) => if(x._1 >= y._1) x else y)
  ...
  val dataByArea = joinedMaxDecades.mapValues{case(a,(b,c)) => (a,b,c)}.map{case(k,v) => k.drop(3).dropRight(2) -> v}
