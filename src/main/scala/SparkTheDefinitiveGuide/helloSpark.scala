...
object HelloSpark extends App {
  ...
  val flightData2015 = spark.read
    ...
    .option("header", "true")
    .csv("data/flight/2015-summary.csv")
  ...
  val sqlway = spark.sql(
    """SELECT DEST_COUNTRY_NAME as dest, sum(count) as flight FROM flight_data_2015 GROUP BY DEST_COUNTRY_NAME ORDER BY flight DESC"""
  )
  ...
  flightData2015
    .groupBy('DEST_COUNTRY_NAME)
    .sum("count")
    .withColumnRenamed("sum(count)", "destCount")
    .sort('destCount.desc)
    .limit(4)
    .show()
